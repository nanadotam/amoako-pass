...
import { ArrowLeft, Eye, EyeOff, Wand2, Plus, X, Globe, Shield, Save } from "lucide-react"
...
interface EditPasswordModalProps {
  ...
  onSave?: (updatedPassword: PasswordData) => void
}
...
export function EditPasswordModal({ 
  ...
  // Pre-populate form data when password changes
  useEffect(() => {
    if (password && open) {
      setFormData({
        ...
        url: password.url || "",
        twoFactorEnabled: password.twoFactorEnabled,
        tags: [...password.tags],
        alternativeEmails: [...password.alternativeEmails],
        securityQuestions: [...password.securityQuestions],
      })
      
      // Find matching platform
      const matchingPlatform = popularPlatforms.find(p => 
        p.name.toLowerCase() === password.website.toLowerCase() || 
        p.url === password.url
      )
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedPassword: PasswordData = {
      ...
      favicon: selectedPlatform?.favicon || password.favicon || "",
    }
    ...
    return (
      <Drawer open={open} onOpenChange={onOpenChange}>
        <DrawerContent className="px-4 pb-4 max-h-[90vh] overflow-y-auto">
          <DrawerHeader>
            <DrawerTitle>Edit Password - {password.website}</DrawerTitle>
          </DrawerHeader>
          {content}
        </DrawerContent>
      </Drawer>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Password - {password.website}</DialogTitle>
        </DialogHeader>
        {content}
      </DialogContent>
    </Dialog>
